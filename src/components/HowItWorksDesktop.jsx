...
import { useHiwActiveStep } from '../motion/hiw-active-step.js'
...
const HIW_TABS = ['Before the meeting', 'In the meeting', 'After the meeting']

export default function HowItWorksDesktop() {
  const sectionRef = useRef(null)
  const step1Ref = useRef(null)
  const { activeIndex, onTabClick } = useHiwActiveStep(sectionRef)
  useHiwStageScale(sectionRef)
  useHiwVideoThumbs(sectionRef)
  // Same two random callers as the hero, picked once per page load.
  const [callers] = useState(pickHeroCallVideos)
  useHiwStepDemo(step1Ref, activeIndex === 1)
  return (
    <section ref={sectionRef} className="how-it-works-progress-scope relative hidden md:grid grid-cols-2 max-w-7xl mx-auto w-full px-4  md:px-10 2xl:px-0">
      <div className="relative">
        <div className="sticky top-24 flex max-w-xs flex-col items-start gap-4 py-12">
          {HIW_TABS.map((label, i) => (
            <button
              key={label}
              type="button"
              onClick={() => onTabClick(i)}
              className={`flex w-fit cursor-pointer flex-col items-start text-left font-melange text-base transition-colors duration-500 ${i === activeIndex ? 'text-content-primary' : 'text-content-tertiary'}`}
            >
              {label}
              <span aria-hidden="true" className="hiw-step-underline relative mt-1.5 h-px w-[174px] bg-oats-neutral-200">
                <span className={`hiw-step-fill absolute -top-px left-0 h-[3px] w-full transition-colors duration-500 hiw-step-fill-${i} ${i < activeIndex ? 'hiw-step-fill-passed bg-content-tertiary' : 'bg-content-primary'}`} />
              </span>
            </button>
          ))}
        </div>
      ...
      <div className="flex flex-col gap-24 py-12">
        <div data-index="0" className="flex flex-col hiw-step-source-0">
        ...
        <div ref={step1Ref} data-index="1" className="flex flex-col hiw-step-source-1">
          <h3 className="font-quadrant text-2xl font-normal leading-[1.05] tracking-[-0.01em] text-ink-primary">
        ...
        <div data-index="2" className="flex flex-col hiw-step-source-2">
          <h3 className="font-quadrant text-2xl font-normal leading-[1.05] tracking-[-0.01em] text-ink-primary">
